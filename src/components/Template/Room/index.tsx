...
const Room = () => {
  ...
  const removeSameCreatedTime = (list: MessageItem[]) => {
    ...
    Object.keys(objByTime).forEach((key) => {
      objByTime[key] = objByTime[key].map((message, index) => ({
        ...message,
        createdAt: index === 0 ? message.createdAt : ''
      }));
    });
        ...
        <$.MessageList ref={messageRef} isGalleryShow={openGallery}>
          {data?.messages.map((message) => (
            <>
              ...
              {removeSameCreatedTime(message.list).map(
                ({ createdAt, type, content, contentType }, index) => (
